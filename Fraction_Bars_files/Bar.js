...
Bar.prototype.selectSplit = function(pointer_loc) {

	this.selectedSplit = this.splitClickedOn(pointer_loc);
};
...
Bar.prototype.splitClickedOn = function(pointer_loc) {
	for( var i = this.splits.length-1; i >= 0; i-- ) {
		// Utilities.log(i);
		if( pointer_loc.x > this.x + this.splits[i].x &&
			pointer_loc.x < this.x + this.splits[i].x + this.splits[i].w &&
			pointer_loc.y > this.y + this.splits[i].y &&
			pointer_loc.y < this.y + this.splits[i].y + this.splits[i].h)
		{
...
Bar.createFrompointer = function(p1, p2, type, color) {
