...
//	this.barFill = '#FFFF66' ;
	this.matFill = '#888888' ;
	this.pointerDownLoc = null ;
	this.pointerUpLoc = null ;
	this.pointerLastLoc = null ;
...
FractionBarsCanvas.prototype.addBar = function(a_bar) {
	...
	if (a_bar === null | a_bar === undefined) {
		b = Bar.createFrompointer(this.pointerDownLoc, this.pointerUpLoc, 'bar', this.currentFill) ;
	} else {
		b = a_bar;
	}
	...
	this.updateCanvas(this.pointerUpLoc);
...
FractionBarsCanvas.prototype.addMat = function() {
	var m = Mat.createFrompointer(this.pointerDownLoc, this.pointerUpLoc, 'mat', this.matFill) ;
	this.mats.push(m);
	this.updateCanvas(this.pointerUpLoc);
	this.refreshCanvas();
	// Utilities.Log(this.bars.length);
...
FractionBarsCanvas.prototype.barClickedOn = function() {
	for( var i = this.bars.length-1; i >= 0; i-- ) {
		// Utilities.log(i);
		if( this.pointerDownLoc.x > this.bars[i].x &&
			this.pointerDownLoc.x < this.bars[i].x + this.bars[i].w &&
			this.pointerDownLoc.y > this.bars[i].y &&
			this.pointerDownLoc.y < this.bars[i].y + this.bars[i].h)
		{
			...
			if (this.currentAction == "manualSplit" ) {
				...
				this.bars[i].splitBarAtPoint(this.pointerDownLoc, split_key);

			} else {
				this.bars[i].selectSplit(this.pointerDownLoc);
			}
			return this.bars[i] ;
		}
	}
...
FractionBarsCanvas.prototype.matClickedOn = function() {
	for( var i = this.mats.length-1; i >= 0; i-- ) {
		// Utilities.log(i);
		if( this.pointerDownLoc.x > this.mats[i].x &&
			this.pointerDownLoc.x < this.mats[i].x + this.mats[i].w &&
			this.pointerDownLoc.y > this.mats[i].y &&
			this.pointerDownLoc.y < this.mats[i].y + this.mats[i].h)
		{
...
FractionBarsCanvas.prototype.updateCanvas = function(currentpointerLoc) {

	if ((this.currentAction == 'bar') || (this.currentAction == 'mat')) {
		if( this.canvasState !== null ) {
			this.context.putImageData(this.canvasState,0,0);
		}
		if( this.pointerDownLoc !== null ) {
			this.drawRect(this.pointerDownLoc, currentpointerLoc) ;
		}
	} else if (this.currentAction == "manualSplit") {
		// this.calculateSplitLine(currentpointerLoc);
		this.manualSplitPoint = currentpointerLoc;
	} else {
		// we're dragging stuff around
		this.drag(currentpointerLoc);
	}
};
...
FractionBarsCanvas.prototype.clearpointer = function() {
	this.pointerDownLoc = null ;
	this.pointerUpLoc = null ;
};

FractionBarsCanvas.prototype.drag = function(currentLoc) {
	if( this.pointerLastLoc === null || typeof(this.pointerLastLoc) == 'undefined') {
		this.pointerLastLoc = this.pointerDownLoc ;
	}

	for( var i = 0; i < this.selectedBars.length; i++ ) {
		this.selectedBars[i].x = this.selectedBars[i].x + currentLoc.x - this.pointerLastLoc.x ;
		this.selectedBars[i].y = this.selectedBars[i].y + currentLoc.y - this.pointerLastLoc.y ;

	}

	for(i = 0; i < this.selectedMats.length; i++ ) {
		this.selectedMats[i].x = this.selectedMats[i].x + currentLoc.x - this.pointerLastLoc.x ;
		this.selectedMats[i].y = this.selectedMats[i].y + currentLoc.y - this.pointerLastLoc.y ;
	...
	this.pointerLastLoc = currentLoc ;
