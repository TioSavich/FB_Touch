...
//		fbContext.restore() ;
	...
	$('#fbCanvas').pointermove(function(e) {
		fracEvent = e;
		updatepointerLoc(e, $(this));
		updatepointerAction('pointermove');

		var p = Point.createFrompointerEvent(e, $(this)) ;
		...
		if(fbCanvasObj.pointerDownLoc !== null) {
...
//		}
	...
	$('#fbCanvas').pointerdown(function(e) {

		fbCanvasObj.check_for_drag = true;
		fbCanvasObj.cacheUndoState();

		updatepointerLoc(e, $(this));
		updatepointerAction('pointerdown');
		fbCanvasObj.pointerDownLoc = Point.createFrompointerEvent(e, $(this)) ;
		...
		if( (fbCanvasObj.currentAction == 'bar') || (fbCanvasObj.currentAction == "mat")) {
			...
			b.repeat(fbCanvasObj.pointerDownLoc);
			fbCanvasObj.refreshCanvas();
		} else {
			...
			if( b !== null ) {
				if( $.inArray(b, fbCanvasObj.selectedBars) == -1) { // clicked on bar is not already selected
					...
					b.isSelected = true;
					b.selectSplit(fbCanvasObj.pointerDownLoc);
	...
	$('#fbCanvas').pointerup(function(e) {
		updatepointerLoc(e, $(this));
		updatepointerAction('pointerup');

		fbCanvasObj.pointerUpLoc = Point.createFrompointerEvent(e, $(this)) ;
		...
		fbCanvasObj.pointerUpLoc = null ;
		fbCanvasObj.pointerDownLoc = null ;
		fbCanvasObj.pointerLastLoc = null ;
...
function updatepointerLoc(e, elem) {
	x = e.clientX - elem.position().left ;
	y = e.clientY - elem.position().top ;
	offsetX = elem.offset().left;
	offsetY	= elem.offset().top;
	/*
	$('#pointerLoc').text(x + ', ' + y + ' | ' + offsetX  + ', ' + offsetY + ' | ' + window.pageXOffset  + ', ' + window.pageYOffset );
	*/
}

function updatepointerAction(actionName) {
	/*
	$('#pointerAction').text(actionName) ;
	*/
}
